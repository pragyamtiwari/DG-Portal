...
def update_user_role(uuid, is_admin):
    conn, curs = get_conn_curs()
    curs.execute("SELECT * FROM users WHERE uuid = ?", (uuid,))
    user = curs.fetchone()
    if not user:
    ...
    curs.execute("UPDATE users SET is_admin = ? WHERE uuid = ?", (is_admin, uuid))
    commit_close(conn, curs)
    return {
        "status": "200",
...
def delete_policy(uuid):
    ...
    # Also delete associated tag_policies
    curs.execute("DELETE FROM tag_policies WHERE policy_uuid = ?", (uuid,))
    commit_close(conn, curs)
    return {
        "status": "200",
...
def delete_tag(uuid):
    ...
    # Also delete associated tag_policies
    curs.execute("DELETE FROM tag_policies WHERE tag_uuid = ?", (uuid,))
    commit_close(conn, curs)
    return {
        "status": "200",
...
def get_user_assignments(user_uuid):
...
def get_tags_with_member_count():
    ...
    curs.execute("""
        SELECT t.*, 
               CASE 
                   WHEN t.members IS NULL OR t.members = '' THEN 0
                   ELSE LENGTH(t.members) - LENGTH(REPLACE(t.members, ',', '')) + 1
               END as member_count,
               COUNT(tp.uuid) as policy_count
        FROM tags t
        LEFT JOIN tag_policies tp ON t.uuid = tp.tag_uuid
        GROUP BY t.uuid
        ORDER BY t.name
    """)
    ...
    return {
        "status": "200",
        "message": "Tags with member and policy count found successfully",
        "data": [dict(tag) for tag in tags]
    }

def create_tag_policy(tag_uuid, policy_uuid, due_date):
    conn, curs = get_conn_curs()
    curs.execute("SELECT uuid FROM tag_policies WHERE tag_uuid = ? AND policy_uuid = ?", (tag_uuid, policy_uuid))
    if curs.fetchone():
        commit_close(conn, curs)
        return {
            "status": "409",
            "message": "Policy already linked to this tag",
            "data": None
        }
    uuid = generate_uuid()
    curs.execute("INSERT INTO tag_policies (uuid, tag_uuid, policy_uuid, due_date) VALUES (?, ?, ?, ?)", (uuid, tag_uuid, policy_uuid, due_date))
    commit_close(conn, curs)
    return {
        "status": "201",
        "message": "Policy linked to tag successfully",
        "data": None
    }

def get_tag_policies(tag_uuid):
    conn, curs = get_conn_curs()
    curs.execute("SELECT * FROM tag_policies WHERE tag_uuid = ?", (tag_uuid,))
    tag_policies = curs.fetchall()
    commit_close(conn, curs)
    return {
        "status": "200",
        "message": "Tag policies found successfully",
        "data": [dict(tp) for tp in tag_policies]
    }

def get_tag_policies_with_details(tag_uuid):
    conn, curs = get_conn_curs()
    curs.execute("""
        SELECT tp.*, p.name as policy_name, p.description as policy_description, tp.due_date
        FROM tag_policies tp
        JOIN policies p ON tp.policy_uuid = p.uuid
        WHERE tp.tag_uuid = ?
        ORDER BY p.name
    """, (tag_uuid,))
    tag_policies = curs.fetchall()
    ...
    return {
        "status": "200",
        "message": "Tag policies with details found successfully",
        "data": [dict(tp) for tp in tag_policies]
    }

def delete_tag_policy(uuid):
    conn, curs = get_conn_curs()
    curs.execute("SELECT * FROM tag_policies WHERE uuid = ?", (uuid,))
    tag_policy = curs.fetchone()
    if not tag_policy:
        commit_close(conn, curs)
        return {
            "status": "404",
            "message": "Tag policy not found",
            "data": None
        }
    curs.execute("DELETE FROM tag_policies WHERE uuid = ?", (uuid,))
    commit_close(conn, curs)
    return {
        "status": "200",
        "message": "Tag policy deleted successfully",
        "data": None
    }

def delete_tag_policies_by_tag(tag_uuid):
    conn, curs = get_conn_curs()
    curs.execute("DELETE FROM tag_policies WHERE tag_uuid = ?", (tag_uuid,))
    commit_close(conn, curs)
    return {
        "status": "200",
        "message": "Tag policies deleted successfully",
        "data": None
    }

def get_policies_by_tag(tag_uuid):
    conn, curs = get_conn_curs()
    curs.execute("""
        SELECT p.*, tp.due_date
        FROM policies p
        JOIN tag_policies tp ON p.uuid = tp.policy_uuid
        WHERE tp.tag_uuid = ?
    """, (tag_uuid,))
    policies = curs.fetchall()
    commit_close(conn, curs)
    return {
        "status": "200",
        "message": "Policies found for tag successfully",
        "data": [dict(policy) for policy in policies]
    }
